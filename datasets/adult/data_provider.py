...
def get_image_label_from_record_tabular(image_record, label_record):
  ...
  # Convert from tf.string to tf.uint8.
  record_defaults = []  
  for idx in range(105):   # adult processed set has 105 columns
...
class UCI_Adult(object):
  ...
  def __init__(self, data_dir, subset, batch_size, is_training=False):
    ...
    if is_training:
        images_file = os.path.join(data_dir, "adult_train_processed.csv")   # full data
        labels_file = os.path.join(data_dir, "adult_train_label.csv")       # full data
    else:
        images_file = os.path.join(data_dir, "adult_test_processed.csv")
        labels_file = os.path.join(data_dir, "adult_test_label.csv")
