  ...
  def __call__(self, images, is_training=False):
    ...
    for i, num_units in enumerate(self.num_units_fc_layers):
      ...
      with tf.variable_scope(
          os.path.join("adult_network", "fc_" + layer_suffix), reuse=reuse):
        net = tf.layers.dense(
            net,
            num_units,
    ...
    with tf.variable_scope(
        os.path.join("adult_network", "output_layer"), reuse=reuse):
