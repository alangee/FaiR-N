...
def get_image_label_from_record_tabular(image_record, label_record):
  ...
  # Convert from tf.string to tf.uint8.
  record_defaults = []  
  for idx in range(139):   # meps processed set has 139 columns
  ...
  # Convert from tf.string to tf.uint8.
  label = tf.decode_csv(label_record, record_defaults=[[1]])
  # Convert from tf.uint8 to tf.int32.
...
class MEPS(object):
  ...
  def __init__(self, data_dir, subset, batch_size, is_training=False):
    ...
    if is_training:
        images_file = os.path.join(data_dir, "meps_train_processed.csv")   # full data
        labels_file = os.path.join(data_dir, "mepsdata_train_labels.txt")       # full data
    else:
        images_file = os.path.join(data_dir, "meps_test_processed.csv")
        labels_file = os.path.join(data_dir, "mepsdata_test_labels.txt")

    
    df = pd.read_csv(images_file, header=1)
    X = df.to_numpy()
